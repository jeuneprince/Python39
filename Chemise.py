...
def connecter():
        ...
        try:
            ...
            cur=con.cursor()
            cur.execute("select nom, description, prix from articles where categories_id=3")
            rows = cur.fetchall()
...
def OnDoubleClick(event):
    item = tree.selection()
    reponse = messagebox.askquestion("Achat", "Voulez vous ajouter " + str(tree.item(tree.selection())['values'][0])+ " a votre panier ?")
    if reponse == 'yes':
        ajoutPanier()
    elif reponse == 'no':
        messagebox.showinfo('Reponse', 'Vous aimerez un autre article certainement')
    else:
        messagebox.showwarning('error', 'Une erreur est apparue ...')
...
root.title("Magasiner des Chemises et T-shirts")
